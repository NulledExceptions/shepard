...
@click.option("--use_env_variables", help="Even if you have a profile set you can set this flag to 'False' and the CLI will act as if you don't have a profile set allowing you to overwrite the profile variables you set.", default='True')
...
@click.option("--dont_assume", help="Set this flag to True to not assume role to use a command. If you've set a role or your working in the root account this would make sense to do. Allowed values are "+'"'+"True"+'" or'+ ' "'+"False"+'".', default='False')
...
def run(command,account_number, use_env_variables, role_to_assume_to_target_account, path_to_docker_folder, ecr_repo_to_push_to, path_to_local_folder_to_batch, s3_bucket_to_upload_to, dynamo_db_to_query, cloudformation_to_describe, path_to_local_secrets, secret_store, profile_name, zip_name_override, s3_bucket_for_results, directory_to_sync_s3_bucket_to, dont_assume, mfa_token, serial_number, lambda_to_invoke, json_payload):
    "Valid commands are: 'deploy','batch','batch_via_api','query','describe','secretify','update','configure','check_update','help','check_profile','set_profile','delete_profile','where_am_i','set_role','release_role', and 'check_role'."
    if not command:
        print()
        ...
        print('Available commands: ' + "'deploy','batch','query','describe','secretify','update','configure','check_update','help','check_profile','set_profile','delete_profile','where_am_i','set_role','release_role','check_role'")
    ...
    if command not in ['deploy','batch','batch_via_api','query','describe','secretify','update','configure','check_update','help','check_profile','set_profile','delete_profile','where_am_i','retrieve','set_role','release_role','check_role']:
        print('"'+command+'"'+ ' was not recognized as a valid command. Valid commands are: ' + "'deploy','batch','batch_via_api','query','describe','secretify','check_update','configure','check_profile','delete_profile','where_am_i','retrieve','set_role','check_role','release_role' and 'set_profile'.")
    ...
    if command == 'deploy':
        ...
        if None in [account_number,role_to_assume_to_target_account,path_to_docker_folder,ecr_repo_to_push_to]:
            print('You have not initialized a variable that needs to be used for the deploy command!')
            raise ValueError('You have not initialized a variable that needs to be used for the deploy command!')
    ...
    if command == 'batch':
        ...
        if None in [account_number,role_to_assume_to_target_account,path_to_local_folder_to_batch,s3_bucket_to_upload_to]:
            print('You have not initialized a variable that needs to be used for the batch command!')
            raise ValueError('You have not initialized a variable that needs to be used for the batch command!')
    ...
    if command == 'query':
        ...
        if None in [account_number,role_to_assume_to_target_account,dynamo_db_to_query]:
            print('You have not initialized a variable that needs to be used for the query command!')
            raise ValueError('You have not initialized a variable that needs to be used for the query command!')

        query(account_number,role_to_assume_to_target_account,dynamo_db_to_query,dont_assume,mfa_token,serial_number)

    if command == 'describe':

        if None in [account_number,role_to_assume_to_target_account,cloudformation_to_describe]:
            print('You have not initialized a variable that needs to be used for the describe command!')
            raise ValueError('You have not initialized a variable that needs to be used for the describe command!')
    ...
    if command == 'secretify':
        ...
        if None in [account_number,role_to_assume_to_target_account,path_to_local_secrets,secret_store]:
            print('You have not initialized a variable that needs to be used for the secretify command!')
            raise ValueError('You have not initialized a variable that needs to be used for the secretify command!')
    ...
    if command == 'configure':
        ...
        if None in [profile_name]:
            print('You have not initialized a variable that needs to be used for the configure command!')
            raise ValueError('You have not initialized a variable that needs to be used for the configure command!')
    ...
    if command == 'set_profile':
        ...
        if None in [profile_name]:
            print('You have not initialized a variable that needs to be used for the set_profile command!')
            raise ValueError('You have not initialized a variable that needs to be used for the set_profile command!')
    ...
    if command == 'delete_profile':
        ...
        if None in [profile_name]:
            print('You have not initialized a variable that needs to be used for the delete_profile command!')
            raise ValueError('You have not initialized a variable that needs to be used for the delete_profile command!')
    ...
    if command == 'retrieve':
        ...
        if None in [account_number,role_to_assume_to_target_account,s3_bucket_for_results,directory_to_sync_s3_bucket_to]:
            print('You have not initialized a variable that needs to be used for the retrieve command!')
            raise ValueError('You have not initialized a variable that needs to be used for the retrieve command!')
    ...
    if command == 'set_role':
        ...
        if None in [account_number, role_to_assume_to_target_account, mfa_token, serial_number]:
            print('You have not initialized a variable that needs to be used for the set_role command!')
            raise ValueError('You have not initialized a variable that needs to be used for the set_role command!')
    ...
    if command == 'batch_via_api':
        ...
        if None in [account_number,role_to_assume_to_target_account,lambda_to_invoke,json_payload]:
            print('You have not initialized a variable that needs to be used for the batch_via_api command!')
            raise ValueError('You have not initialized a variable that needs to be used for the batch_via_api command!')
