...
errors_bucket = str(os.getenv('errors_bucket')) #the bucket error logs get dumped to
...
if __name__ == "__main__":
	...
	try:
		...
		with open('environment_variables.env', 'w+') as the_file:
			...
			the_file.write(str('ERRORS_BUCKET') + '=' + str(errors_bucket) + '\n')
		...
		try:
			###################################################################################################################
			# zip up log folder and push it to S3. also add tree diagram to log folder.
			###################################################################################################################
			#update status and log!
			JOB_STATUS = 'job failed; pushing logs to s3'
			update_item_in_dynamoDB(table_name, region, item_blob, START_TIME, JOB_STATUS, END_TIME)
			logging.log(level=logging.INFO, msg='job failed; pushing logs to s3')

			#Write tree diagram of current directory
			os.chdir('/')
			os.mkdir('crash_log_files_'+UUID)
			os.system("tree " + str(my_dir_root) + " >"+'/crash_log_files_'+UUID+"/tree_root.txt")

			if USES_LUSTRE == "True":
				os.system("tree " + str(my_dir_lustre) + " >"+'/crash_log_files_'+UUID+"/tree_lustre.txt")

			if USES_EFS == "True":
				os.system("tree " + str(my_dir_efs) + " >"+'/crash_log_files_'+UUID+"/tree_efs.txt")

			os.chdir('/')
			shutil.make_archive('/crash_log_files_'+UUID, 'zip', '/crash_log_files_'+UUID)
			link_to_results = upload_to_s3('crash_log_files_'+UUID+'.zip', errors_bucket, 'log@'+UUID+'_'+str(START_TIME)+'.zip')
			logging.log(level=logging.INFO, msg='var directory uploaded to errorlogs')
			rv = check_output('rm -rf ' + '/crash_log_files_'+UUID + ' >/dev/null 2>&1', shell=True)  # kill your directory
